import { Controller, Get, Post, Delete, Param, Query, Res, UseGuards, UseInterceptors, ParseIntPipe, DefaultValuePipe } from '@nestjs/common';
...
export class CacheController extends BaseController {
  ...
  async getInfo() {
      ...
      const commandStats = commandStatsStr.split('\r\n').slice(1,-1).reduce((r,d) => {
        const [key,value] = d.split(':')
        r[key.trim().replace('cmdstat_','')] = value.trim()
        r.push({
          name: key.trim().replace('cmdstat_',''),
          value: this.stringUtils.substringBetween(value.trim(),"calls=", ",usec")
        })
        return r
      },[])
      ...
      const clientInfo = clientInfoStr.split('\r\n').slice(1,-1).reduce((r,d) => {
          const [key,value] = d.split(':')
          r[key.trim()] = value.trim()
          return r
      },{})
      ...
      const memoryInfo = memoryInfoStr.split('\r\n').slice(1,-1).reduce((r,d) => {
          const [key,value] = d.split(':')
          r[key.trim()] = value.trim()
          return r
      },{})
      ...
      const cpuInfo = cpuInfoStr.split('\r\n').slice(1,-1).reduce((r,d) => {
          const [key,value] = d.split(':')
          r[key.trim()] = value.trim()
          return r
      },{})
      ...
      const persistenceInfo = persistenceInfoStr.split('\r\n').slice(1,-1).reduce((r,d) => {
          const [key,value] = d.split(':')
          r[key.trim()] = value.trim()
          return r
      },{})
      ...
      const statsInfo = statsInfoStr.split('\r\n').slice(1,-1).reduce((r,d) => {
          const [key,value] = d.split(':')
          r[key.trim()] = value.trim()
          return r
      },{})
     ...
     const result = {
      info: {
          ...
          instantaneous_input_kbps: parseFloat(statsInfo['instantaneous_input_kbps']), 
          instantaneous_output_kbps: parseFloat(statsInfo['instantaneous_output_kbps']) 
      },
      dbSize: dbSize,
  ...
  async getCacheValue(@Param('cacheName') cacheName: string, @Param('cacheKey') cacheKey: string) {
    const cacheValue:string = await this.redisCacheService.getCacheObject(cacheKey);  
