...
export class SysLogininforController extends BaseController {
  ...
  async list(@Query() query: SysLogininfor, @Request() req) {
    ...
    const [list, total] =
      await this.logininforService.selectLogininforList(query);
  ...
  async export(@Res() res, @Body() logininfor: SysLogininfor, @Request() req) {
    const [list] =
      await this.logininforService.selectLogininforList(logininfor);
  ...
  @Delete(':infoIds')
  async remove(
    @Param('infoIds', new ParseArrayPipe({ items: Number, separator: ',' }))
    infoIds: number[],
  ) {
