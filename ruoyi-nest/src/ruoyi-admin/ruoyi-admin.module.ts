      ...
      load: [
        () => {
          const applicationDruid = yaml.load(
            fs.readFileSync(
              path.join(__dirname, 'resources', 'application-druid.yml'),
              'utf8',
            ),
          ) as Record<string, any>;
          const application = yaml.load(
            fs.readFileSync(
              path.join(__dirname, 'resources', 'application.yml'),
              'utf8',
            ),
          ) as Record<string, any>;
          const gen = yaml.load(
            fs.readFileSync(
              path.join(
                __dirname,
                '../ruoyi-generator/resources/generator.yml',
              ),
              'utf8',
            ),
          ) as Record<string, any>;
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        ...
        extra: {
          ...
          waitForConnections: configService.get(
            'datasource.poolOptions.waitForConnections',
          ),
          queueLimit: configService.get('datasource.poolOptions.queueLimit'),
          connectTimeout: configService.get(
            'datasource.poolOptions.connectTimeout',
          ),
        },
        // logging: true,
