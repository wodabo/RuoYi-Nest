...
export class SysLoginService {
  ...
  private async validateCaptcha(
    ...
    if (captchaEnabled) {
      ...
      const captcha: string =
        await this.redisCacheService.getCacheObject(verifyKey); // TODO: Get from redis cache
  ...
  private async loginPreCheck(
    ...
    // IP黑名单校验
    const blackStr = await this.configService.selectConfigByKey(
