...
export class SysMenuController extends BaseController {
  ...
  async list(@Query() query: SysMenu, @Request() req): Promise<AjaxResult> {
    ...
    const menus = await this.menuService.selectMenuListWithUserId(
      query,
      currentUser.userId,
    );
  ...
  async treeselect(
    ...
    const tree = await this.treeUtils.arrayToTree(
      transformedMenus,
      'id',
      'parentId',
      'children',
  ...
  async add(@Body() menu: SysMenu, @Request() req): Promise<AjaxResult> {
    ...
    if (!(await this.menuService.checkMenuNameUnique(menu))) {
      ...
      !this.stringUtils.isHttp(menu.path)
    ) {
      return this.error(
        `新增菜单'${menu.menuName}'失败，地址必须以http(s)://开头`,
      );
    }
  ...
  async edit(@Body() menu: SysMenu, @Request() req): Promise<AjaxResult> {
    ...
    if (!(await this.menuService.checkMenuNameUnique(menu))) {
      ...
      return this.error(
        `修改菜单'${menu.menuName}'失败，地址必须以http(s)://开头`,
      );
