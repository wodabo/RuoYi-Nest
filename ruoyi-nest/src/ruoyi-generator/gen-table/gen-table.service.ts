...
export class GenTableService {
  ...
  async importGenTable(tableList: GenTable[], operName: string): Promise<void> {
    for (const table of tableList) {
      ...
      const genTableColumns =
        await this.genTableColumnRepository.selectDbTableColumnsByName(
          tableName,
        );
  ...
  async previewCode(tableId: number): Promise<Map<string, string>> {
    ...
    // 获取要渲染的模板列表。
    const templates = this.hbsUtils.getTemplateList(
      table.tplCategory,
      table.tplWebType,
    );
    ...
    for (const file of fileList) {
      ...
      if (file.path.endsWith('.ts')) {
        ...
        if (file.path.endsWith('controller.ts')) {
          ...
          const absPath = await this.fileUploadUtils.getAbsoluteFile(
            controllerPath,
            fileName,
          );
  ...
  async synchDb(tableName: string): Promise<void> {
    ...
    const tableColumnMap = tableColumns.reduce(
      (map, obj) => ((map[obj.columnName] = obj), map),
      {},
    );
    const dbTableColumns =
      await this.genTableColumnRepository.selectDbTableColumnsByName(tableName);
    if (!dbTableColumns) {
      throw new ServiceException(
        'Failed to synchronize data, original table structure does not exist',
      );
    }
    const dbTableColumnNames = dbTableColumns.map(
    ...
    for (const column of dbTableColumns) {
      ...
      if (tableColumnMap.hasOwnProperty(column.columnName)) {
        ...
        if (
          GenTableColumnEntityUtils.isRequired(prevColumn) &&
          !GenTableColumnEntityUtils.isPk(column) &&
          (GenTableColumnEntityUtils.isInsert(prevColumn) ||
            GenTableColumnEntityUtils.isEdit(prevColumn)) &&
          (GenTableColumnEntityUtils.isUsableColumn(prevColumn) ||
            !GenTableColumnEntityUtils.isSuperColumn(prevColumn))
        ) {
    ...
    const delColumns = tableColumns.filter(
      (column) => !dbTableColumnNames.includes(column.columnName),
    );
    if (delColumns) {
    ...
    if (GenConstants.TPL_TREE === genTable.tplCategory) {
      ...
      if (!paramsObj[GenConstants.TREE_CODE]) {
        ...
        if (!genTable.subTableName) {
          ...
          throw new ServiceException(
            'Sub table foreign key name cannot be empty',
          );
        }
      }
    }
  ...
  public async setSubTable(table: GenTable): Promise<void> {
    ...
    if (subTableName) {
      table.subTable =
        await this.genTableRepository.selectGenTableByName(subTableName);
    }
  }
