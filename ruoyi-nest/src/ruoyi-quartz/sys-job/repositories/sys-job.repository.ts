...
export class SysJobRepository {
  ...
  async selectJobList(job: SysJob): Promise<[SysJob[], number]> {
    ...
    if (job.jobName) {
      queryBuilder.andWhere('job.jobName LIKE :jobName', {
        jobName: `%${job.jobName}%`,
      });
    }

    if (job.jobGroup) {
      queryBuilder.andWhere('job.jobGroup = :jobGroup', {
    ...
    if (job.invokeTarget) {
      queryBuilder.andWhere('job.invokeTarget LIKE :invokeTarget', {
  ...
  async selectJobById(jobId: number): Promise<SysJob> {
    const queryBuilder = this.selectJobVo().where('job.jobId = :jobId', {
      jobId,
    });
