...
export class PermissionValidatorService {
    ...
    hasPermi(permission: string,loginUser:LoginUser): boolean {
        ...
        if (!loginUser || !loginUser.permissions || loginUser.permissions.length === 0) {
            return false;
        }
    ...
    hasAnyPermi(permissions: string,loginUser:LoginUser): boolean {
        ...
        for (const permission of permissions.split(Constants.PERMISSION_DELIMETER)) {
    ...
    hasRole(role: string,loginUser:LoginUser): boolean {
        ...
        if (!loginUser || !loginUser.user.roles || loginUser.user.roles.length === 0) {
            return false;
        }
    ...
    hasPermissions(permissions: string[], permission: string): boolean {
        return permissions.includes(Constants.ALL_PERMISSION) || permissions.includes(permission.trim());
    }
}