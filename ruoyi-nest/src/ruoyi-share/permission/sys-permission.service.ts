...
export class SysPermissionService {
  ...
  async getRolePermission(user: SysUser): Promise<string[]> {
    ...
    if (this.securityUtils.isAdmin(user)) {
      ...
      const userRoles = await this.roleService.selectRolePermissionByUserId(
        user.userId,
      );
      ...
      if (roles && roles.length > 0) {
        ...
        for (const role of roles) {
          if (role.status === UserConstants.ROLE_NORMAL) {
            const rolePerms = await this.menuService.selectMenuPermsByRoleId(
              role.roleId,
            );
        ...
        const userPerms = await this.menuService.selectMenuPermsByUserId(
          user.userId,
        );
