...
export class FileUploadUtils {
  ...
  async upload(file: Express.Multer.File): Promise<string> {
    try {
      return await this.uploadWithExtension(
        this.getDefaultBaseDir(),
        file,
        this.mimeTypeUtils.DEFAULT_ALLOWED_EXTENSION,
      );
    } catch (e) {
  ...
  private async assertAllowed(
    ...
    if (size > this.DEFAULT_MAX_SIZE) {
      throw new FileSizeLimitExceededException(
        this.DEFAULT_MAX_SIZE / 1024 / 1024,
      );
    }
    ...
    if (
      allowedExtension &&
      !this.isAllowedExtension(extension, allowedExtension)
    ) {
