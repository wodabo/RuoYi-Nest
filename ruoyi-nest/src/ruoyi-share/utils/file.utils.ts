...
export class FileUtils {
     ...
     * @param os 输出流
     */ 
    public async writeBytes(filePath: string, os: NodeJS.WritableStream): Promise<void> {
        const fs = require('fs');
    ...
    public async writeImportBytes(data: Buffer): Promise<string> {
        try {
            return await this.writeBytesToFile(data, this.ruoYiConfigService.getImportPath());
        } catch (e) {
     ...
     * @throws IOException IO异常
     */
    public async writeBytesToFile(data: Buffer, uploadDir: string): Promise<string> {
        try {
            // 获取文件扩展名
            ...
            // 获取绝对路径文件
            const file = await this.fileUploadUtils.getAbsoluteFile(uploadDir, pathName);
    ...
    public checkAllowDownload(resource: string): boolean {
        ...
        // 检查允许下载的文件规则
        if (this.mimeTypeUtils.DEFAULT_ALLOWED_EXTENSION.includes(this.fileTypeUtils.getFileTypeByFileName(resource))) {
            return true;
        }
     ...
     * @return 编码后的文件名
     */ 
    public async setFileDownloadHeader(request: Request, fileName: string): Promise<string> {
     ...
     */
    public async setAttachmentResponseHeader(response, realFileName: string): Promise<void> {
        ...
        response.header('access-control-expose-headers', 'Content-Disposition,download-filename');
     ...
     */
     public fileListToZip(files: Array<{path: string, content: string}>): Promise<Buffer> {
