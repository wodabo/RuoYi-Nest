...
export class GenUtils {
  ...
  public initTable(genTable: GenTable, operName: string): void {
    ...
    // 举例 system
    genTable.moduleName = this.getModuleName(
      this.genConfigService.getPackageName(),
    );
    // 举例 config
  ...
  public initColumnField(column: GenTableColumn, table: GenTable): void {
    ...
    if (
      GenConstants.COLUMNTYPE_STR.includes(dataType) ||
      GenConstants.COLUMNTYPE_TEXT.includes(dataType)
    ) {
      // 字符串长度超过500设置为文本域
      const columnLength = this.getColumnLength(column.columnType);
      const htmlType =
        columnLength >= 500 || GenConstants.COLUMNTYPE_TEXT.includes(dataType)
      ...
      const str = StringUtils.substringBetween(
        column.columnType,
        '(',
        ')',
      ).split(',');
    ...
    // 编辑字段
    if (
      !GenConstants.COLUMNNAME_NOT_EDIT.includes(columnName) &&
    ...
    // 列表字段
    if (
      !GenConstants.COLUMNNAME_NOT_LIST.includes(columnName) &&
      !GenTableColumnEntityUtils.isPk(column)
    ) {
    ...
    // 查询字段
    if (
      !GenConstants.COLUMNNAME_NOT_QUERY.includes(columnName) &&
      !GenTableColumnEntityUtils.isPk(column)
    ) {
  ...
  public convertClassName(tableName: string): string {
    const autoRemovePre = this.genConfigService.getAutoRemovePre();
    const tablePrefix = this.genConfigService.getTablePrefix();
    if (autoRemovePre && StringUtils.isNotEmpty(tablePrefix)) {
      const searchList = tablePrefix.split(',');
  ...
  public getColumnLength(columnType: string): number {
    if (columnType.indexOf('(') > 0) {
      const length = StringUtils.substringBetween(columnType, '(', ')');
