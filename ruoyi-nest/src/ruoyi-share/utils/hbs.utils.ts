...
export class HbsUtils {
  ...
  public prepareContext(genTable: GenTable): any {
    ...
    hbsContext.functionName = functionName
      ? functionName
      : '【请填写功能名称】';
    ...
    hbsContext.permissionPrefix = this.getPermissionPrefix(
      moduleName,
      businessName,
    );
  ...
  public getFileName(template: string, genTable: GenTable): string {
    ...
    if (
      template.includes('sub-entity.ts.hbs') &&
      GenConstants.TPL_SUB === genTable.tplCategory
    ) {
  ...
  public getImportList(genTable: GenTable): Set<string> {
    ...
    for (const column of columns) {
      if (
        !GenTableColumnEntityUtils.isSuperColumn(column) &&
        GenConstants.TYPE_DATE === column.tsType
      ) {
        importList.add('java.util.Date');
        importList.add('com.fasterxml.jackson.annotation.JsonFormat');
      } else if (
        !GenTableColumnEntityUtils.isSuperColumn(column) &&
        GenConstants.TYPE_BIGDECIMAL === column.tsType
      ) {
  ...
  public getParentMenuId(paramsObj): string {
    if (
      ...
      paramsObj.hasOwnProperty(GenConstants.PARENT_MENU_ID) &&
