...
export class SysMenuRepository {
  ...
  async selectMenuList(query: SysMenu): Promise<SysMenu[]> {
    ...
    if (query.menuName) {
      queryBuilder.andWhere('m.menuName LIKE :menuName', {
    ...
    const [row, total] = await this.queryUtils.executeQuery(
      queryBuilder,
      query,
    );
    return row;
    ...
    if (menuCheckStrictly) {
      queryBuilder.andWhere(
        'm.menuId NOT IN ' +
          ...
          'INNER JOIN sys_role_menu rm ON m.menuId = rm.menu_id AND rm.role_id = :roleId)',
        { roleId },
      );
    }
  ...
  async selectMenuById(menuId: number): Promise<SysMenu> {
    const queryBuilder = this.selectMenuVo().where('m.menuId = :menuId', {
      menuId,
    });
  ...
  async insertMenu(menu: SysMenu): Promise<number> {
    ...
    if (menu.parentId != null && menu.parentId != 0)
      insertObj.parentId = menu.parentId;
    if (menu.menuName != null && menu.menuName != '')
      insertObj.menuName = menu.menuName;
    ...
    if (menu.component != null && menu.component != '')
      insertObj.component = menu.component;
    ...
    if (menu.isFrame != null && menu.isFrame != '')
      insertObj.isFrame = menu.isFrame;
    if (menu.isCache != null && menu.isCache != '')
      insertObj.isCache = menu.isCache;
    if (menu.menuType != null && menu.menuType != '')
      insertObj.menuType = menu.menuType;
    ...
    if (menu.createBy != null && menu.createBy != '')
      insertObj.createBy = menu.createBy;
  ...
  async updateMenu(menu: SysMenu): Promise<number> {
    ...
    // 只有当字段有值时才添加到更新对象中
    if (menu.menuName != null && menu.menuName != '')
      updateData.menuName = menu.menuName;
    ...
    if (menu.isFrame != null && menu.isFrame != '')
      updateData.isFrame = menu.isFrame;
    if (menu.isCache != null && menu.isCache != '')
      updateData.isCache = menu.isCache;
    if (menu.menuType != null && menu.menuType != '')
      updateData.menuType = menu.menuType;
    ...
    if (menu.updateBy != null && menu.updateBy != '')
      updateData.updateBy = menu.updateBy;
