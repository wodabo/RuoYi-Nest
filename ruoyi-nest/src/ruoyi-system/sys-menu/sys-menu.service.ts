...
export class SysMenuService {
   ...
   * @returns 菜单列表
   */
  async selectMenuListWithUserId(
    menu: SysMenu,
    userId: number,
  ): Promise<SysMenu[]> {
  ...
  buildMenus(menus: SysMenu[]): RouterVo[] {
    ...
    for (const menu of menus) {
      ...
      if (
        cMenus &&
        cMenus.length > 0 &&
        UserConstants.TYPE_DIR === menu.menuType
      ) {
        ...
        children.name = this.getRouteNameByNameAndPath(
          menu.routeName,
          routerPath,
        );
        children.meta = new MetaVo(this.stringUtils);
  ...
  public innerLinkReplaceEach(path: string): string {
    return this.stringUtils.replaceEach(
      path,
      [Constants.HTTP, Constants.HTTPS, Constants.WWW, '.', ':'],
      ['', '', '', '/', '/'],
    );
  }
  ...
  public getComponent(menu: SysMenu): string {
    ...
    if (
      this.stringUtils.isNotEmpty(menu.component) &&
      !this.isMenuFrame(menu)
    ) {
      component = menu.component;
    } else if (
      this.stringUtils.isEmpty(menu.component) &&
      menu.parentId !== 0 &&
  ...
  async selectMenuListByRoleId(roleId: number): Promise<number[]> {
    ...
    return this.menuRepository.selectMenuListByRoleId(
      roleId,
      role.menuCheckStrictly,
    );
  }
