...
export class SysPostRepository {
  ...
  async selectPostList(query: SysPost): Promise<[SysPost[], number]> {
    ...
    if (query.postCode) {
      queryBuilder.andWhere('p.postCode LIKE :postCode', { postCode: `%${query.postCode}%` });
    }
    if (query.postName) {
      queryBuilder.andWhere('p.postName LIKE :postName', { postName: `%${query.postName}%` });
    }
    if (query.status) {
  ...
  async checkPostNameUnique(postName: string): Promise<SysPost> {
    const queryBuilder = this.selectPostVo()
      .where('p.postName = :postName', { postName });
