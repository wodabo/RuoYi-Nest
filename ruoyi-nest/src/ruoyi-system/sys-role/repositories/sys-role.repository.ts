...
export class SysRoleRepository {
  ...
  async selectRoleList(query: SysRole): Promise<[SysRole[], number]> {
    ...
    if (query.roleName) {
      queryBuilder.andWhere('r.roleName like :roleName', { roleName: `%${query.roleName}%` });
    }
    if (query.status) {
      queryBuilder.andWhere('r.status = :status', { status: query.status });
    }
    if (query.roleKey) {
      queryBuilder.andWhere('r.roleKey like :roleKey', { roleKey: `%${query.roleKey}%` });
    }
    if (query.params?.beginTime) {
      queryBuilder.andWhere('DATE_FORMAT(r.createTime,"%Y%m%d") >= DATE_FORMAT(:beginTime,"%Y%m%d")', {
        beginTime: query.params.beginTime
      });
    }
    if (query.params?.endTime) {
      queryBuilder.andWhere('DATE_FORMAT(r.createTime,"%Y%m%d") <= DATE_FORMAT(:endTime,"%Y%m%d")', {
  ...
  async selectRolePermissionByUserId(userId: number): Promise<SysRole[]> {
    const queryBuilder = this.roleRepository.createQueryBuilder('r')
      ...
      .select([
        'r.roleId', 'r.roleName', 'r.roleKey', 'r.roleSort', 'r.dataScope',
        'r.menuCheckStrictly', 'r.deptCheckStrictly', 'r.status', 'r.delFlag',
        'r.createTime', 'r.remark', 'r.createBy', 'r.updateBy', 'r.updateTime'
      ])
      .where('r.delFlag = :delFlag', { delFlag: '0' })
  ...
  async updateRole(role: SysRole): Promise<number> {
    ...
    // 只有当字段有值时才添加到更新对象中
    if (role.roleName != null && role.roleName != '') updateData.roleName = role.roleName;
    if (role.roleKey != null && role.roleKey != '') updateData.roleKey = role.roleKey;
    if (role.roleSort != null) updateData.roleSort = role.roleSort;
    if (role.dataScope != null && role.dataScope != '') updateData.dataScope = role.dataScope;
    if (role.menuCheckStrictly != null) updateData.menuCheckStrictly = role.menuCheckStrictly;
    if (role.deptCheckStrictly != null) updateData.deptCheckStrictly = role.deptCheckStrictly;
    if (role.status != null && role.status != '') updateData.status = role.status;
