...
export class SysRoleService {
  ...
  async selectRolesByUserId(
    userId: number,
    loginUser?: LoginUser,
  ): Promise<SysRole[]> {
    const userRoles =
      await this.roleRepository.selectRolePermissionByUserId(userId);
  ...
  async checkRoleNameUnique(role: SysRole): Promise<boolean> {
    ...
    const existingRole = await this.roleRepository.checkRoleNameUnique(
      role.roleName,
    );
  ...
  async checkRoleKeyUnique(role: SysRole): Promise<boolean> {
    const roleId = role.roleId ? role.roleId : -1;
    const existingRole = await this.roleRepository.checkRoleKeyUnique(
      role.roleKey,
    );
