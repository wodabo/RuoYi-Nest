...
export class SysUserRepository {
  ...
  async selectUserList(query: SysUser): Promise<[SysUser[], number]> {
    ...
    if (query.params?.beginTime && query.params?.endTime) {
      queryBuilder.andWhere(
        "DATE_FORMAT(u.createTime,'%Y%m%d') BETWEEN DATE_FORMAT(:beginTime,'%Y%m%d') AND DATE_FORMAT(:endTime,'%Y%m%d')",
        {
  ...
  async selectUnallocatedList(query: SysUser): Promise<[SysUser[], number]> {
    const queryBuilder = this.userRepository
      ...
      .andWhere(
        'u.userId NOT IN (SELECT u.userId FROM sys_user u INNER JOIN sys_user_role ur ON u.userId = ur.userId AND ur.roleId = :roleId)',
        { roleId: query.roleId },
      );
  ...
  async selectUserById(userId: number): Promise<SysUser> {
    const queryBuilder = this.selectUserVo().where('u.userId = :userId', {
      userId,
    });
  ...
  async insertUser(user: SysUser): Promise<number> {
    ...
    const queryBuilder = entityManager
      .createQueryBuilder()
      .insert()
      .into(SysUser, Object.keys(insertObject))
      .values(insertObject);
  ...
  async registerUser(user: SysUser): Promise<number> {
    ...
    if (user.userName != null && user.userName != '')
      insertObj.userName = user.userName;
    if (user.nickName != null && user.nickName != '')
      insertObj.nickName = user.nickName;
    if (user.email != null && user.email != '') insertObj.email = user.email;
    if (user.avatar != null && user.avatar != '')
      insertObj.avatar = user.avatar;
    if (user.phonenumber != null && user.phonenumber != '')
      insertObj.phonenumber = user.phonenumber;
    if (user.sex != null && user.sex != '') insertObj.sex = user.sex;
    if (user.password != null && user.password != '')
      insertObj.password = user.password;
    if (user.status != null && user.status != '')
      insertObj.status = user.status;
    if (user.createBy != null && user.createBy != '')
      insertObj.createBy = user.createBy;
    if (user.remark != null && user.remark != '')
      insertObj.remark = user.remark;
  ...
  async updateUser(user: SysUser): Promise<number> {
    ...
    if (user.loginIp !== null && user.loginIp !== '')
      updateData.loginIp = user.loginIp;
    if (user.updateBy !== null && user.updateBy !== '')
      updateData.updateBy = user.updateBy;
    if (user.remark !== null && user.remark !== '')
      updateData.remark = user.remark;
