...
export class SysUserService {
  ...
  async importUser(
    ...
    const password = await this.configService.selectConfigByKey(
    ...
    for (const user of userList) {
      try {
        ...
        if (!existUser) {
          ...
          successNum++;
          successMsgs.push(
            `<br/>${successNum}、账号 ${user.userName} 导入成功`,
          );
          ...
          successNum++;
          successMsgs.push(
            `<br/>${successNum}、账号 ${user.userName} 更新成功`,
          );
        } else {
          failureNum++;
